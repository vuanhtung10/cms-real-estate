...
;(function($) {
  ...
  const Mapplic = function(element) {
    const self = this
    ...
    function Tooltip() {
      ...
      this.init = function() {
        ...
        if (this.tooltipthumb)
          this.thumbnail = $('<img>')
            .addClass('mapplic-tooltip-thumbnail')
            .hide()
            .appendTo(this.el)
      ...
      this.position = function() {
        if (this.location) {
          ...
          const x = this.location.x * 100
          const y = this.location.y * 100
          let mt = -this.el.outerHeight() - this.shift
          let ml = -this.el.outerWidth() / 2
      ...
      this.zoom = function(location) {
        let ry = 0.5
        const zoom = location.zoom ? parseFloat(location.zoom) : self.o.maxscale

        ry =
          ((self.container.height() - this.drop) / 2 + this.drop) /
    ...
    function HoverTooltip() {
      ...
      this.init = function() {
        ...
        $(self.map)
          .on('mouseover', '.mapplic-layer a', function() {
            ...
            const location = self.getLocationData(id)
        ...
        if (self.o.selector) {
          $(self.map)
            .on('mouseover', self.o.selector, function() {
              const location = self.getLocationData($(this).attr('id'))
              s.shift = 20
              if (location) s.show(location)
            })
            .on('mouseout', function() {
      ...
      this.position = function(location) {
        ...
        const x = location.x * 100
        const y = location.y * 100
        let mt = -this.el.outerHeight() - this.shift
        let ml = 0
        ...
        if (Math.abs(mt) > cy) {
          mt = 8 + 2
          if (this.pin && this.pin.length)
            mt = this.pin.height() + parseFloat(this.pin.css('margin-top')) + 20
          this.el.addClass('mapplic-bottom')
        } else this.el.removeClass('mapplic-bottom')
    ...
    function DeeplinkingHash() {
      ...
      this.check = function(easing) {
        const id = location.hash.slice(this.param.length + 2)
        self.showLocation(id, easing, true)
      }
    ...
    function Minimap() {
      ...
      this.update = function(x, y) {
        const active = $('.mapplic-minimap-active', this.el)
    ...
    function Sidebar() {
      ...
      this.search = function(keyword) {
        ...
        $('.mapplic-list > li', self.el).each(function() {
          const count = $('.mapplic-list-shown', this).length
    ...
    function DevTools() {
      ...
      this.init = function() {
        ...
        $('.mapplic-layer', self.map).on('mousemove', function(e) {
          const x = (e.pageX - self.map.offset().left) / self.map.width()
          const y = (e.pageY - self.map.offset().top) / self.map.height()
    ...
    var processData = function(data) {
        ...
        $.each(data.levels, function(index, level) {
              ...
              $('<div></div>')
                ...
                .load(source, function() {
                  ...
                  $('svg a', this).each(function() {
                    const location = self.getLocationData(
                      $(this)
                        .attr('xlink:href')
                        .substr(1)
                    )
          ...
          $.each(level.locations, function(index, location) {
            ...
            if (location.lat && location.lng) {
              const pos = latlngToPos(location.lat, location.lng)
              location.x = pos.x
              location.y = pos.y
            }

            const top = location.y * 100
            const left = location.x * 100

            if (!location.pin) location.pin = 'default'
            if (!location.pin.includes('hidden')) {
              if (self.o.markers) {
                const pin = $('<a></a>')
                  .attr('href', '#')
      ...
      $(window)
        .resize(function() {
          ...
          if ($(window).width() < 668) {
            if (self.el.hasClass('mapplic-fullscreen'))
              ...
              const height = Math.min(
                Math.max(
                  (self.container.width() * self.contentHeight) /
                    self.contentWidth,
                  ($(window).height() * 2) / 3
                ),
                $(window).height() - 66
          ...
          const wr = self.container.width() / self.contentWidth
          const hr = self.container.height() / self.contentHeight

          if (self.o.mapfill) {
            if (wr > hr) self.fitscale = wr
            else self.fitscale = hr
          } else if (wr < hr) self.fitscale = wr
          else self.fitscale = hr
      ...
      if (self.o.landmark) {
        ...
        const init = self.getLocationData('init') // Landmark with ID 'init'
    ...
    var addControls = function() {
      const map = self.map
      const mapbody = $('.mapplic-map-image', self.map)
      ...
      mapbody.on('mousedown', function(e) {
        ...
        self.map.on('mousemove', function(e) {
          ...
          let x = e.pageX - map.data('mouseX') + self.x
          let y = e.pageY - map.data('mouseY') + self.y
      ...
      self.el.on('dblclick', '.mapplic-map-image', function(e) {
        ...
        self.x = normalizeX(
          self.x -
            (e.pageX - self.container.offset().left - self.x) *
              (self.scale / scale - 1)
        )
        self.y = normalizeY(
          self.y -
            (e.pageY - self.container.offset().top - self.y) *
              (self.scale / scale - 1)
        )
      ...
      if (self.o.mousewheel) {
        $('.mapplic-layer', self.el).bind('mousewheel DOMMouseScroll', function(
          e,
          delta
        ) {
          const scale = self.scale
      ...
      mapbody.on('touchstart', function(e) {
        ...
        mapbody.on('touchmove', function(e) {
          ...
          const orig = e.originalEvent
          const touches = orig.touches.length
      ...
      // Pinch zoom
      const hammer = new Hammer(self.map[0], {
    ...
    var latlngToPos = function(lat, lng) {
      const deltaLng = self.data.rightLng - self.data.leftLng
      const bottomLatDegree = (self.data.bottomLat * Math.PI) / 180
      const mapWidth = ((self.data.mapwidth / deltaLng) * 360) / (2 * Math.PI)
      const mapOffsetY =
        (mapWidth / 2) *
        Math.log(
    ...
    const addClass = function(element, c) {
      ...
      if (!classes.includes(c)) element.attr('class', classes + ' ' + c)
    }

    const removeClass = function(element, c) {
      const classes = element.attr('class')
      if (classes) element.attr('class', classes.replace(c, '').trim())
    }

    const hasClass = function(element, c) {
      const classes = element.attr('class')
    ...
    var normalizeX = function(x) {
      const minX = self.container.width() - self.contentWidth * self.scale
    ...
    var normalizeY = function(y) {
      const minY = self.container.height() - self.contentHeight * self.scale
    ...
    self.switchLevel = function(target) {
      ...
      const layer = $('.mapplic-layer[data-floor="' + target + '"]', self.el)
      ...
      // Update control
      const index = self.levelselect.get(0).selectedIndex
      const up = $('.mapplic-levels-up', self.el)
      const down = $('.mapplic-levels-down', self.el)
        ...
        $.each(level.locations, function(index, location) {
          if (location.id == id) {
            ...
            switch (action) {
              ...
              case 'select':
                if (location.el) {
                  if (hasClass(location.el, 'mapplic-active'))
                    removeClass(location.el, 'mapplic-active')
  ...
  $.fn.mapplic = function(options) {
    return this.each(function() {
      ...
      const instance = new Mapplic(element).init(options)
