...
;(function($) {
  $.idleTimer = function(firstParam, elem) {
    ...
    const jqElem = $(elem)
    let obj = jqElem.data('idleTimerObj') || {}
    /* (intentionally not documented)
     * Toggles the idle state and fires an appropriate event.
     * @return {void}
     */
    const toggleIdleState = function(e) {
      const obj = $.data(elem, 'idleTimerObj') || {}
      ...
      const event = $.Event((obj.idle ? 'idle' : 'active') + '.idleTimer')
    ...
    const handleEvent = function(e) {
      ...
      if (e.type === 'mousemove') {
        ...
        const elapsed = +new Date() - obj.olddate
        if (elapsed < 200) {
          return
        }
      }
    ...
    const remainingtime = function() {
      ...
      let remaining = obj.timeout - (+new Date() - obj.lastActive)
      if (remaining < 0) {
        remaining = 0
      }
     ...
     */
    jqElem.on(
      $.trim((opts.events + ' ').split(' ').join('._idleTimer ')),
      function(e) {
        handleEvent(e)
      }
    )
