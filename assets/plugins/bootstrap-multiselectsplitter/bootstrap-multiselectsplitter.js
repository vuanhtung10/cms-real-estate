...
;+(function($) {
  ...
  const MultiSelectSplitter = function(element, options) {
  ...
  MultiSelectSplitter.DEFAULTS = {
    ...
    maximumAlert(maximumSelected) {
      alert('Only ' + maximumSelected + ' values can be selected')
    },
    createFirstSelect(label, $originalSelect) {
      return '<option>' + label + '</option>'
    },
    createSecondSelect(label, $firstSelect) {
      return '<option>' + label + '</option>'
    },
    template:
  ...
  MultiSelectSplitter.prototype.init = function(type, element, options) {
    ...
    self.$wrapper = self.$element.next(
    ...
    self.$element.find('optgroup').each(function() {
      ...
      const $option = $(self.options.createFirstSelect(label, self.$element))

      $option.val(label)
      $option.attr('data-current-label', $option.text())
      self.$firstSelect.append($option)

      const currentOptionCount = $(this).find('option').length
    ...
    let selectSize = Math.max(optgroupCount, longestOptionCount)
    ...
    self.$secondSelect.on(
      'click change',
      $.proxy(self.updateChildCategory, self)
    )
    ...
    self.update = function() {
      ...
      const selectedOptions = self.$element.find('option:selected:first')
      let selectedGroup

      if (selectedOptions.length) {
        selectedGroup = selectedOptions.parent().attr('label')
      } else {
        selectedGroup = self.$element
          .find('option:first')
          .parent()
          .attr('label')
      }
      ...
      self.$firstSelect
        .find('option[value="' + selectedGroup + '"]')
        .prop('selected', true)
  ...
  MultiSelectSplitter.prototype.createSecondSelect = function() {
    ...
    $.each(
      self.$element.find(
        'optgroup[label="' + self.$firstSelect.val() + '"] option'
      ),
      function(index, element) {
        ...
        const $option = $(
          self.options.createSecondSelect(label, self.$firstSelect)
        )
        $option.val(value)
  ...
  MultiSelectSplitter.prototype.updateCounter = function() {
    ...
    $.each(self.$firstSelect.find('option'), function(index, element) {
      const originalLabel = $(element).val()
      let text = $(element).data('currentLabel')
      const count = self.$element.find(
        'optgroup[label="' + originalLabel + '"] option:selected'
      ).length
  ...
  MultiSelectSplitter.prototype.checkSelected = function() {
    ...
    const $actualElementSelected = self.$element.find('option:selected')
    ...
    if ($actualElementSelected.length > maximumSelected) {
      ...
      const firstSelectedOptGroupLabel = self.last$ElementSelected
        .first()
        .parent()
        .attr('label')
      self.$firstSelect
        .find('option[value="' + firstSelectedOptGroupLabel + '"]')
        .prop('selected', true)
  ...
  MultiSelectSplitter.prototype.basicUpdateChildCategory = function(
    event,
    isCtrlKey
  ) {
    const self = this
    ...
    self.last$ElementSelected = self.$element.find('option:selected')
    let childValues = self.$secondSelect.val()

    if (!$.isArray(childValues)) {
      childValues = [childValues]
    }

    const parentLabel = self.$firstSelect.val()
    let removeActualSelected = false

    if (!self.$element.attr('multiple')) {
      removeActualSelected = true
    } else if (self.options.onlySameGroup) {
      $.each(self.$element.find('option:selected'), function(index, element) {
        if (
          $(element)
            .parent()
            .attr('label') != parentLabel
        ) {
          removeActualSelected = true
          return false
        }
        ...
        if (
          parentLabel ==
            $(element)
              .parent()
              .attr('label') &&
          $.inArray($(element).val(), childValues) == -1
        ) {
          $(element).prop('selected', false)
        }
      })
  ...
  function Plugin(option) {
    return this.each(function() {
      const $this = $(this)
      let data = $this.data('multiselectsplitter')
      ...
      if (!data) {
        $this.data(
          'multiselectsplitter',
          (data = new MultiSelectSplitter(this, options))
        )
      }
