...
;('use strict')
// plugin setup
const KTPortlet = function(elementId, options) {
  // Main object
  let the = this
  const init = false
  ...
  var Plugin = {
    ...
    build() {
      // Remove
      const remove = KTUtil.find(the.head, '[data-ktportlet-tool=remove]')
      if (remove) {
      ...
      // Reload
      const reload = KTUtil.find(the.head, '[data-ktportlet-tool=reload]')
      if (reload) {
      ...
      // Toggle
      const toggle = KTUtil.find(the.head, '[data-ktportlet-tool=toggle]')
      if (toggle) {
    ...
    fullscreen(mode) {
      ...
      if (
        ...
        if (the.foot) {
          const height1 = parseInt(KTUtil.css(the.foot, 'height'))
          const height2 =
